...
RUN git clone https://github.com/mpx/lua-cjson.git /tmp/lua-cjson \
    && apt install -y gcc libc6-dev liblua5.1-0-dev \
    && cd /tmp/lua-cjson \
    && make CFLAGS="-I/usr/include/lua5.1" \
    && cp cjson.so /tmp/packages/

FROM ${NGINX_FROM_IMAGE}
RUN --mount=type=bind,target=/tmp/packages/,source=/tmp/packages/,from=builder \
    apt-get update \
    && mkdir -p /usr/local/lib/lua/5.1/ \
    && cp /tmp/packages/cjson.so /usr/local/lib/lua/5.1/ \
